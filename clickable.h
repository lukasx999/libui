...
template <>
struct std::formatter<ui::Clickable::State> : std::formatter<std::string> {
    auto format(const ui::Clickable::State& state, std::format_context& ctx) const {

        auto fmt = [&] {
            switch (state) {
                using enum ui::Clickable::State;
